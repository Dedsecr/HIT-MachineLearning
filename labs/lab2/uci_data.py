...
'''
def get_uci_data_wine():
    file_path = './uci_data/wine.data'
    with open(file_path, 'r') as f:
        lines = [l[:-1].split(',') for l in f.readlines()]
        np.random.shuffle(lines)
    X = np.array([[float(l[i]) for i in range(1, len(l))] for l in lines])
    Y = np.array([[int(l[i]) - 1 for i in range(1)] for l in lines])
    legal_index = []
    for i in range(len(Y)):
        if Y[i] <= 1:
            legal_index.append(i)
    X, Y = X[legal_index], Y[legal_index]
    train_num = int(X.shape[0] * 0.3)
    test_num = X.shape[0] - train_num
    train_data = X[:train_num], Y[:train_num]
    test_data = X[test_num:], Y[test_num:]
    return train_data, test_data
'''
